...
addi x20, x0, 2                 # filter address
slli x20, x20, 8                # 5x5 filter stored in m[512]
...
kernel_load:
    ...
    addi x0, x0, 0
    addi x0, x0, 0
    vslidedown.vi v12, v12, 5   # drop first 2 elements of row4 (stride = 2)
...
convolution:                    # performs convolution on 5x5 matrix
    ...
    vle32.v v24, x20            # load filter
    addi x0, x0, 0
    addi x0, x0, 0
    addi x0, x0, 0
    addi x0, x0, 0
    vmul.vv v20, v20, v24       # dot product
    vmv.s.x v24, x0             # v29[0] = 0: Drop first element of filter matrix
