        ...
        if (args==2):
            if (syntax=='r-l'): 
                ...
                try:
                    ...
                    exit()
            elif (syntax=='r-r' or syntax=='r-v' or syntax=='v-r'): #edited
                try:
                ...
                if ('C.' in inst[0]):
                    if (inst_info['comp_reg']):
                        if ((arg2<8) | (arg2>15)):
                            ...
                            exit()
            if (syntax=='r-v'):
                inst = [inst[0], arg1, 0 , arg2]
            elif (syntax=='v-r'):
                inst = [inst[0], arg1, arg2, 0]
            else:
            ...
            arg1 = register_dict[inst[1]]
            if ((syntax=='r-r-l') | (syntax=='r-r-i') | (syntax=='r-r-r') | (syntax=='r-r')):
                try:
                    arg2 = register_dict[inst[2]]
                except IndexError:
                    print('Invalid second arguement (Reg) on {}'.format(inst))
                ...
                if (syntax=='r-r-l'):
                    ...
                    try:
                        ...
                        exit()
                elif (syntax=='r-r'): #edited
                    try:
                        arg2 = 0
                        arg3 = register_dict[inst[2]]
                    except IndexError:
                        print('Invalid second arguement (Reg) on {}'.format(inst))
                        exit()
                else:
                ...
                if (encoding_type=='CLS'):
                    if ((arg3<8) | (arg3>15)):
                        ...
                        exit()
            if ('v0.t' in inst): #if there is masking
                inst = [inst[0], arg1, arg3, arg2, 'v0.t']
            elif (inst_info['format']=='V'):
                inst = [inst[0], arg1, arg3, arg2]
            else:
    ...
    for inst_address in instructions.keys():
        ...
        #new
            ...
            if (temp_inst[-1]=='v0.t'): #if there is masking
                mask=1
            m_code = opcode |  d<<7 | funct3<<12 | s1<<15 | s2<<20 | (funct7-mask)<<25
        
        elif (encoding_type=='VI'):
            d = int(temp_inst[1])
            s1 = int(temp_inst[2])
            s2 = int(temp_inst[3])
            m_code = opcode |  d<<7 | funct3<<12 | s1<<15 | s2<<20 
        
        elif (encoding_type=='VII'):
            d = int(temp_inst[1])
            s1 = int(temp_inst[2])
            s2 = int(temp_inst[3])
            m_code = opcode |  d<<7 | funct3<<12 | s1<<15 | s2<<20 | 3<<30
